...
for fixture in stress goodmorning; do
    bun scripts/benchmark.ts run --fixture ".kajji-benchmarks/fixtures/$fixture" --runs 3 --warmups 1 --scenarios diff --steps 20 --passes 1 --output "$out/$fixture.json" >"$out/$fixture.log" 2>&1 || { tail -60 "$out/$fixture.log"; exit 1; }
done
python3 - "$out" <<'PY'
