...
export function DiffFileHeader(props: { file: FlattenedFile; maxWidth: number }) {
    ...
    const previousName = () => (props.file.prevName ? ` ← ${props.file.prevName}` : "")
    const statusColor = () =>
        getStatusColor(getDiffStatusKey(props.file.type as DiffFileStatus), colors())
    const headerText = () =>
        truncatePathMiddle(
            `${props.file.name}${previousName()}`,
            Math.max(1, props.maxWidth - statsWidth() - FILE_HEADER_PREFIX.length - 1),
        )
        ...
        >
            ...
            <text wrapMode="none" flexShrink={0}>
                <Show when={props.file.additions > 0}>
                    <span style={{ fg: ADDITION_COLOR }}>+{props.file.additions}</span>
                </Show>
                <Show when={props.file.additions > 0 && props.file.deletions > 0}>
                    <span> </span>
                </Show>
                <Show when={props.file.deletions > 0}>
                    <span style={{ fg: DELETION_COLOR }}>-{props.file.deletions}</span>
                </Show>
            </text>
        </box>
    )
